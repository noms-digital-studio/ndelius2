...
const PageSelection = ({ pageSize, pageNumber, total, gotoPage, searchTerm, probationAreasFilter, searchType }) => {
  const PageLink = ({ linkPageNumber, tabIndex }) => {
    if (pageNumber === linkPageNumber) {
      return (<span className='govuk-body-s govuk-!-margin-right-2'>{ linkPageNumber }</span>)
    }
    return (
      <a tabIndex={ tabIndex } href='#offender-results' title={ `Page ${ linkPageNumber }` }
         className='govuk-link govuk-link--no-visited-state govuk-!-margin-right-2'
    ...
    <div role='navigation'>
      ...
      <span>
        ...
        <span>
          ...
          <span className='govuk-!-margin-right-2'>-</span>
        </span>
        }
